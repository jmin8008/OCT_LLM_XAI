#!/usr/bin/env python3
"""
APTOS 2021 AutoResearch Exp #6: ConvNeXt-Tiny + MIL Max Pool + MixUp + HRF pos_weight tuning
Exp #5 대비: HRF pos_weight를 0.5로 cap (95.7% prevalence 과도반영 완화),
더 큰 batch(16)로 gradient 안정화,更强 MixUp(alpha=0.3), warmup=10.
...
- Loss: BCEWithLogitsLoss with pos_weight (HRF capped at 0.5)
- MixUp: alpha=0.3, probability=0.5
- Exp #6: ConvNeXt-Tiny + MixUp + HRF pw_tuning, lr=2e-4, warmup=10, dropout 0.3, wd 1e-4, batch 16
"""
...
def compute_pos_weights(bags):
    ...
    for i in range(NUM_CLASSES):
        ...
        weights.append(w)
    # HRF (index 3) is majority class (95.7% prevalence) — cap pos_weight at 0.5
    # to prevent model from being biased toward always predicting HRF positive
    weights[3] = min(weights[3], 0.5)
    return weights
...
def main():
    ...
    print(f"Model: ConvNeXt-Tiny + MIL Max Pool + MixUp + HRF pw_tuning")
