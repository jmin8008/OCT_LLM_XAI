#!/usr/bin/env python3
"""
APTOS 2021 AutoResearch Exp #10: ConvNeXt-Tiny + MIL Max Pool + MixUp + 384x384
Exp #5(0.9475) 기반: 이미지 해상도 224→384로 증가.
OCT 원본이 632x596이므로 224로 다운샘플링하면 미세한 망막층 구조(IRF/SRF/PED) 손실.
384x384로 더 많은 디테일 보존 → AUC 개선 기대.
BATCH_SIZE 8→4로 축소 (VRAM 여유 확보).
...
- Exp #10: ConvNeXt-Tiny + MixUp + 384x384, lr=1e-4, warmup=5, dropout=0.3, wd 1e-4, batch=4
"""
...
NUM_CLASSES = 4
IMG_SIZE = 384          # 224 → 384: finer OCT detail
BATCH_SIZE = 4          # 8 → 4: compensate for larger images
MAX_BAG_SIZE = 32
NUM_EPOCHS = 100
LEARNING_RATE = 1e-4
WEIGHT_DECAY = 1e-4
...
def main():
    ...
    print(f"Model: ConvNeXt-Tiny + MIL Max Pool + MixUp + 384x384 (dropout=0.3)")
