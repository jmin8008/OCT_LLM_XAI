#!/usr/bin/env python3
"""
APTOS 2021 AutoResearch Exp #7: ConvNeXt-Tiny + MIL Max Pool + MixUp + Batch 16 + Dropout 0.5
Exp #6 대비: HRF pos_weight cap 제거 (1.0 이하 cap은 HRF AUC 붕괴 유발),
Dropout 0.5로 과적합 지연, MixUp alpha=0.2 (0.3은 variance 과다), warmup=5 (10은 너무 느림).
Batch 16은 Exp #6에서 긍정적이었음.
...
- Feature dim: 768
- Head: LayerNorm -> Linear(768,512) -> GELU -> Dropout(0.5) -> Linear(512,4)
- MIL aggregation: Max Pool across bag
- Loss: BCEWithLogitsLoss with pos_weight (HRF NOT capped below 1.0)
- MixUp: alpha=0.2, probability=0.5
- Exp #7: ConvNeXt-Tiny + MixUp + Batch 16 + Dropout 0.5, lr=2e-4, warmup=5, wd 1e-4
"""
...
def main():
    ...
    # Model
    model = MILClassifier(dropout=0.5).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model: ConvNeXt-Tiny + MIL Max Pool + MixUp + Batch 16 + Dropout 0.5")
