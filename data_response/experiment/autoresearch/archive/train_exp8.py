...
def main():
    ...
    print(f"Model: ConvNeXt-Tiny + MIL Max Pool + MixUp + SWA (dropout=0.3)")
    ...
    swa_start = WARMUP_EPOCHS + 1  # epoch 6
    ...
    # --- SWA final validation ---
    print("\nUpdating BN stats for SWA model...")
    torch.optim.swa_utils.update_bn(train_loader, swa_model, device=device)

    swa_val_loss, swa_preds, swa_labels = validate(swa_model, val_loader, criterion, device)
    swa_auc_dict, swa_mean_auc = compute_auc(swa_labels, swa_preds)
    print(f"swa_val_mean_auc: {swa_mean_auc:.4f}, irf: {swa_auc_dict['IRF']:.4f}, "
          f"srf: {swa_auc_dict['SRF']:.4f}, ped: {swa_auc_dict['PED']:.4f}, hrf: {swa_auc_dict['HRF']:.4f}")

    # Use SWA result if better
    if swa_mean_auc > best_auc:
        best_auc = swa_mean_auc
        best_epoch = -1  # indicates SWA
        best_auc_dict = swa_auc_dict.copy()
        print(f"SWA model is better! Using SWA result.")
