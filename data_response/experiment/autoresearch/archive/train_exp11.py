#!/usr/bin/env python3
"""
APTOS 2021 AutoResearch Exp #11: ConvNeXt-Base + MIL Max Pool + MixUp
Exp #5(0.9475) 기반: backbone을 ConvNeXt-Tiny(28M) → ConvNeXt-Base(88M)로 교체.
더 큰 백본으로 feature 품질 향상. 224x224 유지 (epoch 수 확보).
feature_dim 768→1024, hidden_dim 512→768. batch=4 (VRAM 보상).

Uses prepare.py harness for data loading, splitting, bag building, and AUC.
The agent is free to modify this file entirely.

Key architecture:
- Backbone: timm convnext_base (ImageNet pretrained, 88M params)
- Feature dim: 1024
- Head: LayerNorm -> Linear(1024,768) -> GELU -> Dropout(0.3) -> Linear(768,4)
- MIL aggregation: Max Pool across bag
- Loss: BCEWithLogitsLoss with pos_weight
- MixUp: alpha=0.2, probability=0.5
- Exp #11: ConvNeXt-Base + MixUp, lr=1e-4, warmup=5, dropout=0.3, wd 1e-4, batch=4
"""
...
NUM_CLASSES = 4
IMG_SIZE = 224
BATCH_SIZE = 4          # 8→4: ConvNeXt-Base is ~3x larger
MAX_BAG_SIZE = 32
NUM_EPOCHS = 100
LEARNING_RATE = 1e-4
WEIGHT_DECAY = 1e-4
...
# ===========================================================================
# Model: ConvNeXt-Base + MIL (Max Pooling)
# ===========================================================================

class MILClassifier(nn.Module):
    """ConvNeXt-Base backbone + MIL head with max pooling aggregation."""
    ...
    def __init__(self, backbone_name="convnext_base",
        ...
        self.num_features = self.backbone.num_features  # 1024 for convnext_base
...
def main():
    ...
    # Model
    model = MILClassifier(backbone_name="convnext_base", hidden_dim=768, dropout=0.3).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model: ConvNeXt-Base + MIL Max Pool + MixUp (dropout=0.3)")
    print(f"Total params: {total_params:,}, Trainable: {trainable_params:,}")
    print(f"MixUp: alpha={MIXUP_ALPHA}, prob={MIXUP_PROB}")
    print(f"Backbone: convnext_base, feature_dim: {model.num_features}, hidden_dim: 768, batch: {BATCH_SIZE}")
