...
- **429행** = factual 218 + counterfactual 211. eye-level split: **train 360행(183 eye) /
  test 69행(35 eye)**. test는 factual 35만 평가.
...
- **분할:** 218 eye를 **eye-level split (train 183 / test 35)**. 같은 안구 pre/post는 같은 fold.
- **검증셋(라벨無):** 정량평가 제외.
- **메트릭(셀별 report):** decision_balanced_acc, continue_rate, biomarker_node_acc,
  prognosis_node_acc + majority, response_goodpoor_balacc(rGPbal) + resp3, cf_flip_rate,
  negctrl_flip_rate, faithfulness_gap, text_kg_align.
- **유의성(신규, HANDOFF 다음작업 #4):** test n=35 소표본 → 핵심 셀(tier3 B responder
  rGPbal 0.613, tier2 A_meta rGPbal 0.758)에 **bootstrap 95% CI + permutation test**를 붙여
  표에 **n.s. 명시**. 셀 간 단순 비교는 질적 패턴(3백본 반복)으로만 주장.
...
| **E-A1 biomarker instill** | A→B biomarker_node_acc 상승(3백본) | (A) | matrix |
