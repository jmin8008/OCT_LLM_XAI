...
## 3. Dataset
- APTOS-2021 OCT, pre/post, `continue injection`/composite responder/ΔCST + 바이오마커.
- **두 평가 단위(EXPERIMENTAL_PROTOCOL §4):** ① **Per-slice Biomarker** ~1501 pre-slice
  (CSV presence 라벨; absent IRF 197·SRF 909·PED 1213), ② **Eye-level CDSS** 218 eye
  (최소포함조건 stratified split, **test≈70**).
- 분포·편향(DME/Avastin)·한계(2시점→longitudinal 불가, dry/IRF-absent 희소, eye-level N 제한).
...
> **결과는 평가 단위로 두 표 분리(EXPERIMENTAL_PROTOCOL §4.4):**
> **Table A [Per-slice Biomarker]**(지각, per-slice) · **Table B [Eye-level CDSS]**(결정, eye-level).
> 발견 라벨 (A)~(E)는 DESIGN §6·`matrix.md`와 1:1.

- **(A) biomarker instill → Table A(Per-slice), Fig 2a:** A→B 상승 — **IRF/SRF/PED 분리
  보고**, 균형인 **SRF·PED가 진짜 instill 증거**, IRF(present-편향 0.87) caveat. (집계
  0.32→0.81은 부록으로만.)
- **(B)~(E) → Table B(Eye-level CDSS), 3 백본 × arm × meta:**
  - **(B) decision collapse + 정보론적 천장** → Fig 2b: B/C cont 0.94–1.0, D cont 0.0,
    balAcc 0.42–0.50 (백본무관); 예후 ≈0.257 ≤ majority 0.314.
...
## 7. Limitations
- **eye-level 검정력:** 결정 N이 eye 수에 제한(현 v0.3 test 35 → 재실행 test≈70, 최소포함
  조건으로 dry/IRF-absent 보존). 핵심 셀 bootstrap CI 명시, 3백본 질적 반복으로 방어.
- **per-slice 상관:** 같은 eye 슬라이스는 독립 아님 → eye-clustered CI로 보고(증강은 결정
  태스크 N으로 카운트 금지).
- **dry-eye 검정력:** dry eye(has_fluid=0)는 전체 7개뿐 → 최소포함조건(test ≥2)으로 수학적
  붕괴는 막되, dry/no_active 관련 결론은 근본적으로 약한 검정력임을 정직하게 명시(arm D
  반증·(C2) 천장 해석에 한정 적용).
...
| **Table A** | **Per-slice Biomarker (backbone × arm; IRF/SRF/PED 분리, per-slice)** | (A) | E-A1 |
| **Table B** | **Eye-level CDSS (backbone × arm × meta; decision/responder/prognosis…)** | (B)~(E) / C1·C3 | E-A (matrix.md) |
| Fig 2a | biomarker instill A→B (per-biomarker, SRF·PED 핵심) | (A) | E-A1 |
